...
export const Response = (res, status, body) => {
    if (1 && res && status) {
        const { APPNAME, APPACCESKEY } = process.env
        const sts = parseInt(status);
        res.setHeader("_fromBalabalaapp", APPACCESKEY);
        ...
        switch (sts) {
            ...
            case 503:
                res.status(503).json({
                    status: 503,
                    message: "An SQL error was occured !",
                    data: body ? body : {}
                })
                break;
            ...
            case 501:
                res.status(501).json({
                    status: 501,
                    message: "An unhandled internal server error occured !",
                    data: body ? body : {}
                })
                break;
